<div class="w3-responsive w3-card-4 w3-padding-16" >
    ...
    <table style="width:60%" align='center'>
        ...
        <form method="post" action='<?php echo base_url('LaporanControl/tahunan'); ?>' onsubmit="return validasi_input(this)">
            ...
            <div>
                <input style="height: 40px;width:30%;margin-top:15px" type='text' id='tahun' name='thn' class="form-control" title="Pilih tahun untuk melihat laporan ditahun tersebut" placeholder="Pencarian Tahun" required="">
    ...
    <form class="scroll">
        <table class="w3-table" align="center" style="width:90%;margin-bottom:5%;">
            ...
            <tbody>
                ...
                $index = 1;
                if ($tampil == 0) {
                    echo "<tr class='w3-card'><td style='text-align: center;background-color:white' colspan='4'>"
                    ...
                    if ($ket == 'instrumen') {
                        ...
                        if (isset($laporan_tahunan)) {
                            foreach ($laporan_tahunan as $r):
                                echo "
                                    <tr>
                                    <td style='text-align: left;background-color:white' class='w3-card'><b>$index.</b></td>
                                    <td style='text-align: left;background-color:white' class='w3-card'><b>$r->nama_instrumen</b></td>
                                    <td style='text-align: center;background-color:white' class='w3-card'>$r->JUMLAH_PINJAM</td>
                                    <td class='w3-card'></td>
                                    </tr>";
                                $index++;
                            endforeach;
                        ...
                        if (isset($laporan_tahunan)) {
                            foreach ($laporan_tahunan as $value => $r):
                                $setting = $r->nama_set;
                                $jumlah = $r->jumlah_set;
                                if ($nama_setting == $setting || $nama_setting == NULL) {
                                    ...
                                    <tr>
                                    <td style='text-align: left;background-color:white' class='w3-card'><b>$index.</b></td>
                                    <td style='text-align: left;background-color:white' class='w3-card'><b>$nama_setting</b></td>
                                    <td style='text-align: center;background-color:white' class='w3-card'>$hasil</td>
                                    <td class='w3-card'></td>
                                    </tr>";
                                    $nama_setting = $setting;
                                    $hasil = 0 + $jumlah;
                        ...
                        if (isset($laporan_tahunan)) {
                            ...
                            foreach ($laporan_tahunan as $r):
                                $banyak_pinjam = $this->Peminjaman->laporan_tahunan_banyak_peminjaman($r->id_peminjam, $tahun);
                                $banyak_rusak = $this->Peminjaman->laporan_tahunan_banyak_rusak($r->id_peminjam, $tahun);
                                if($banyak_rusak == 0){
                                    $banyak_rusak = 'Tidak Ada';
                                }
                                
                                $banyak_hilang = $this->Peminjaman->laporan_tahunan_banyak_hilang($r->id_peminjam, $tahun);
                                if($banyak_hilang == 0){
                        ...
                        if (isset($laporan_tahunan)) {
                            foreach ($laporan_tahunan as $r):
                                ...
                                if ($nama == NULL || $nama == $instrumen) {
                                    ...
                                    <td style='text-align: left;background-color:white' class='w3-card'>$nama</td>
                                    <td style='text-align: center;background-color:white' class='w3-card'>$baik</td>
                                    <td style='text-align: center;background-color:white' class='w3-card'>$rusak</td>
                                    <td style='text-align: center;background-color:white' class='w3-card'>$hilang</td>"
                                    . "<td class='w3-card'></td></tr>";
                                    $index++;
                                    $nama = $instrumen;
                                    if ($keterangan == "Rusak") {
                                        $rusak = 1;
