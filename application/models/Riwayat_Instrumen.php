...
class Riwayat_Instrumen extends CI_Model {
    ...
    function laporan_bulanan($bln) {
        $query = "SELECT c.nama_instrumen, c.jumlah AS 'baik', a.keterangan "
                . "FROM riwayat_instrumen a "
                . "join peminjaman b on (a.id_transaksi = b.id_transaksi) "
                . "join instrumen c on (a.id_instrumen = c.id_instrumen) "
                . "WHERE DATE_FORMAT(a.waktu_cek, '%m/%Y')='$bln' GROUP by a.nomor_riwayat order by c.nama_instrumen";
        $hasil=$this->db->query($query);
        return $hasil->result();
    }
    ...
    function laporan_tahunan($thn) {
        $query = "SELECT c.nama_instrumen, c.jumlah AS 'baik', a.keterangan "
                ...
                . "join peminjaman b on (a.id_transaksi = b.id_transaksi) "
                . "join instrumen c on (a.id_instrumen = c.id_instrumen) "
                . "WHERE DATE_FORMAT(a.waktu_cek, '%Y')='$thn' GROUP by a.nomor_riwayat order by c.nama_instrumen";
        $hasil=$this->db->query($query);
        return $hasil->result();
    }
